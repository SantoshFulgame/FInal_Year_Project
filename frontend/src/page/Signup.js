...
function Signup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { firstName, email, password, confirmPassword, addressLine1, city, pincode, state, country, image } = data;
    if (!image) {
         toast.error("Error uploading image. Please try again."); // Display toast error message
      return;
    }
