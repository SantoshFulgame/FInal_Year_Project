...
function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const productResponse = await axios.get(`${process.env.REACT_APP_SERVER_DOMIN}/api/product`);
      const orderResponse = await axios.get(`${process.env.REACT_APP_SERVER_DOMIN}/api/orders`);
      // You need to implement the endpoint for fetching total revenue
      // const revenueResponse = await axios.get(`${process.env.REACT_APP_SERVER_DOMIN}/revenue`);
      ...
      // Set total revenue once the endpoint is implemented
    ...
    <div className="flex flex-col justify-center items-center h-full px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
        ...
        <DashboardCard
          ...
          title={"Total Revenue"}
          value={totalRevenue} // Replace with the actual value of total revenue
        />
        <DashboardCard
